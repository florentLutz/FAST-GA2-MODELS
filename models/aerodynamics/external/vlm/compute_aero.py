...
class ComputeLocalReynolds(ExplicitComponent):
    ...
    def setup(self):
        ...
        if self.options["low_speed_aero"]:
            self.add_output("data:aerodynamics:wing:low_speed:reynolds", val=np.nan)
            self.add_output("data:aerodynamics:horizontal_tail:low_speed:reynolds", val=np.nan)
        else:
            self.add_output("data:aerodynamics:wing:cruise:reynolds", val=np.nan)
            self.add_output("data:aerodynamics:horizontal_tail:cruise:reynolds", val=np.nan)
