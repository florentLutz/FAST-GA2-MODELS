...
class _ComputeAirfoilLiftCurveSlope(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        index_wing = int(math.floor(len(wing_alpha) / 2.))
        wing_airfoil_cl_alpha_array = (wing_cl[1:index_wing] - wing_cl[0]) / (wing_alpha[1:index_wing] - wing_alpha[0])
        ...
        index_htp = int(math.floor(len(htp_alpha) / 2.))
        htp_airfoil_cl_alpha_array = (htp_cl[1:index_htp] - htp_cl[0]) / (htp_alpha[1:index_htp] - htp_alpha[0])
        ...
        index_vtp = int(math.floor(len(vtp_alpha) / 2.))
        vtp_airfoil_cl_alpha_array = (vtp_cl[1:index_vtp] - vtp_cl[0]) / (vtp_alpha[1:index_vtp] - vtp_alpha[0])
