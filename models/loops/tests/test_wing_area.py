...
from models.loops.update_wing_area import UpdateWingArea
...
def test_update_wing_area():
    ...
    assert_allclose(problem["data:aerodynamics:aircraft:landing:additional_CL_capacity"], 0.61, atol=1e-2)
    assert_allclose(problem.get_val("data:weight:aircraft:additional_fuel_capacity", units="kg"), -27, atol=1)
    ...
    assert_allclose(problem["data:aerodynamics:aircraft:landing:additional_CL_capacity"], 0.0, atol=1e-2)
    assert_allclose(problem.get_val("data:weight:aircraft:additional_fuel_capacity", units="kg"), 273, atol=1)
