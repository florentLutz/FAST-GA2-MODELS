...
class StructuralLoadsX57(AerostructuralLoadX57):
    ...
    def setup(self):
        ...
        self.add_input("data:aerodynamics:wing:low_speed:Y_vector", val=nans_array_orig_vector, shape=SPAN_MESH_POINT,
                       units="m")
        self.add_input("data:aerodynamics:wing:low_speed:chord_vector", val=nans_array_orig_vector,
        ...
        self.add_output("data:loads:structure:ultimate:force_distribution:wing", units="N/m",
                        shape=SPAN_MESH_POINT_LOADS)
        self.add_output("data:loads:structure:ultimate:force_distribution:fuel", units="N/m",
                        shape=SPAN_MESH_POINT_LOADS)
        self.add_output("data:loads:structure:ultimate:force_distribution:point_mass", units="N/m",
                        shape=SPAN_MESH_POINT_LOADS)

        self.add_output("data:loads:structure:ultimate:shear:wing", units="N", shape=SPAN_MESH_POINT_LOADS)
        self.add_output("data:loads:structure:ultimate:shear:fuel", units="N", shape=SPAN_MESH_POINT_LOADS)
        self.add_output("data:loads:structure:ultimate:shear:point_mass", units="N", shape=SPAN_MESH_POINT_LOADS)

        self.add_output("data:loads:structure:ultimate:root_bending:wing", units="N*m", shape=SPAN_MESH_POINT_LOADS)
        self.add_output("data:loads:structure:ultimate:root_bending:fuel", units="N*m", shape=SPAN_MESH_POINT_LOADS)
        self.add_output("data:loads:structure:ultimate:root_bending:point_mass", units="N*m", shape=SPAN_MESH_POINT_LOADS)
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        y_vector, _ = self.insert_in_sorted_array(y_vector, 0.)
        ...
        y_vector, point_mass_array_orig = self.compute_relief_force_x57(inputs, y_vector_orig, chord_vector_orig,
                                                                        0., 0.)
        _, wing_mass_array_orig = self.compute_relief_force_x57(inputs, y_vector_orig, chord_vector_orig, wing_mass, 0.,
                                                            False)
        _, fuel_mass_array_orig = self.compute_relief_force_x57(inputs, y_vector_orig, chord_vector_orig, 0., fuel_mass,
                                                            False)
        ...
        outputs["data:loads:structure:ultimate:force_distribution:point_mass"] = point_mass_array_outputs

        point_shear_array = self.compute_shear_diagram(y_vector, load_factor_shear * point_mass_array_orig)
        wing_shear_array = self.compute_shear_diagram(y_vector, load_factor_shear * wing_mass_array_orig)
        fuel_shear_array = self.compute_shear_diagram(y_vector, load_factor_shear * fuel_mass_array_orig)
        ...
        point_root_bending_array = self.compute_bending_moment_diagram(y_vector,
                                                                       load_factor_rbm * point_mass_array_orig)
        wing_root_bending_array = self.compute_bending_moment_diagram(y_vector,
                                                                      load_factor_rbm * wing_mass_array_orig)
        fuel_root_bending_array = self.compute_bending_moment_diagram(y_vector,
                                                                      load_factor_rbm * fuel_mass_array_orig)
